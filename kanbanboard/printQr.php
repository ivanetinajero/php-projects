...
for ($index = 0; $index < count($data); $index++) {
   ...
   $pdf->SetFillColor(255, 255, 255);
   $pdf->setTextColor(0, 0, 0);
   $pdf->MultiCell(35, 60, $data[$index]->name, 1, 'L', 1, 0, $x + 2, $y + 20, true); // descripcion del Ticket
   $pdf->SetFont('times', '', 10);
   
   // Enviamos el codigo QR
   $pdf->write2DBarcode($data[$index]->shortUrl, 'QRCODE,Q', $x + 40, $y + 6, 46, 46, $style, 'N');

   // Numero de Ticket
   //$pdf->SetFillColor(0, 102, 0);
   //$pdf->setTextColor(255, 255, 255);
   //$pdf->MultiCell(40, 5, "Card # " . $data[$index]->shortLink, 1, 'L', 1, 0, $x + 40, $y + 55, true); // Numero de Ticket

   //$pdf->SetFillColor(255, 255, 255);
   //$pdf->setTextColor(0, 0, 0);
   //$pdf->SetFont('times', '', 10);
   //$pdf->MultiCell(25, 5, date('d-m-Y H:i:s', strtotime($data[$index]->dateLastActivity)), 0, 'L', 1, 0, $x + 60, $y + 75, true); // Fecha
